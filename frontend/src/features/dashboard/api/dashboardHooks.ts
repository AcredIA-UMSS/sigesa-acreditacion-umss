 ...
 */
export async function downloadReportFile(jobId: string, defaultFilename: string) {
...
export function useExportReport() {
  ...
  const exportReport = async (format: 'xlsx' | 'csv' | 'pdf', phaseId?: number) => {
    ...
    try {
      // 1. Enqueue job
      const response = await enqueueExportJob({ format, phaseId });
      ...
      await downloadReportFile(jobId, `reporte_coordinador.${ext}`);
    } catch (err: any) {
