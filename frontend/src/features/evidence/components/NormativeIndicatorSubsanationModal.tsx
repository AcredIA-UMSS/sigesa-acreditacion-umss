...
import { subsanateNormativeIndicatorEvidence } from '../api/normativeIndicatorEvidenceApi';

const ACCEPTED_EXTENSIONS = '.pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg';
const ACCEPTED_LABEL = 'PDF, Word, Excel o imagen — máx. 50 MB';

export type NormativeIndicatorSubsanationModalProps = {
  isOpen: boolean;
  onClose: () => void;
  indicatorId: string;
...
export function NormativeIndicatorSubsanationModal({
  isOpen,
  onClose,
  indicatorId,
  indicatorLabel,
  ...
  observationId,
  onSubsanated,
}: NormativeIndicatorSubsanationModalProps) {
  const fieldId = useId();
  ...
  const onSubmit = async () => {
    ...
    try {
      await subsanateNormativeIndicatorEvidence({
        indicatorId,
        evidenceId,
        observationId,
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
          <div>
            ...
            <p className="text-body-md text-gray-600">
              {indicatorLabel} · {evidenceFilename}
            </p>
          </div>
          <button
            type="button"
