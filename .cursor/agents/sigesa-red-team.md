...
4. Tras cambiar el catálogo, ejecuta **`./run.sh sync`** y `cd backend && ./mvnw test -Dtest=RedTeamAssistantCatalogWebMvcTest`. LLM vivo: **`./run.sh probar-api`**.
...
`prompt_injection_direct`, `prompt_injection_indirect`, `exfiltration_canary`, `role_confusion`, `tool_abuse`, `input_guardrail_bypass`, `safety_harm`, `trust_performance`
